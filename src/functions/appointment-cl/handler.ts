...
export const main: SQSHandler = async (event) => {
  console.log('CL - Processing appointment requests:', JSON.stringify(event, null, 2));
  ...
  for (const record of event.Records) {
    try {
      ...
      console.log('CL - Processing appointment:', message.appointmentId);

      await processAppointmentCL(message);
      ...
      console.log(`CL - Appointment ${message.appointmentId} processed successfully`);
    } catch (error) {
      console.error('CL - Error processing appointment:', error);
      ...
      try {
        ...
        console.error('CL - Error publishing failure event:', publishError);
      }
    }
  }

  console.log(`CL - Processing complete. Success: ${successfulMessages.length}, Failed: ${failedMessages.length}`);

  if (failedMessages.length > 0) {
    throw new Error(`CL - Failed to process ${failedMessages.length} messages`);
  }
};


async function processAppointmentCL(message: SNSMessage): Promise<void> {
  console.log('CL - Saving appointment to RDS:', message.appointmentId);
  ...
  console.log('CL - Applying Chile-specific business rules...');

  
  console.log('CL - Appointment saved successfully in RDS');
}
